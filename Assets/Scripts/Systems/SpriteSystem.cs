...
public partial class SpriteSystem : SystemBase
{
    ...
    private ComputeBuffer flipFlagsBuffer;
    private uint[] args;
    ...
    protected override void OnStartRunning()
    {
        ...
        flipFlagsBuffer = new ComputeBuffer(spriteCount, sizeof(int));
        ...
        if (defaultMaterial != null)
        {
            ...
            int flipFlagsBufferId = Shader.PropertyToID("flipFlagsBuffer");
            defaultMaterial.SetBuffer(flipFlagsBufferId, flipFlagsBuffer);
        }
        else
        {
    ...
    {
        // Try to get sorted entities from SpriteSortingSystem
        bool hasSortedEntities = SystemAPI.TryGetSingleton<SortedSprites>(out var sortedEntities);
        ...
        var flipFlags = new NativeArray<int>(spriteCount, Allocator.TempJob);

        if (hasSortedEntities && sortedEntities.Entities.IsCreated && sortedEntities.Entities.Length == spriteCount)
        {
            // Use sorted order
            for (int i = 0; i < spriteCount; i++)
            {
                var entity = sortedEntities.Entities[i];
                var spriteData = EntityManager.GetComponentData<SpriteData>(entity);
                
                translationAndRotations[i] = spriteData.TranslationAndRotation;
                scales[i] = spriteData.Scale;
                colors[i] = spriteData.Color;
                uvIndices[i] = spriteData.UVIndex;
                flipFlags[i] = (int)spriteData.FlipFlags;
            }
        }
        else
        {
            // Fallback to unsorted (original behavior)
        ...
        flipFlagsBuffer.SetData(flipFlags);
    ...
    private struct CollectDataJob : IJobParallelFor
    {
        ...
        public NativeArray<int> FlipFlags;
        ...
        public void Execute(int index)
        {
            ...
            FlipFlags[index] = (int)spriteData.FlipFlags;
        }
    }

    private static readonly Bounds BOUNDS = new Bounds(Vector3.zero, Vector3.one * 1000f);
}